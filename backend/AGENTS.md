...
- `GET /api/labs/{labId}/challenges/{challengeId}/students` — paginated roster of students with a `submission_challenge_result` row for that challenge (submitters only; **score** is highest qualifying challenge score before deadline; **attempts** / **submittedAt** from latest graded attempt)

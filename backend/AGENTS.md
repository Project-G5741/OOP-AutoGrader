...
- `GET /api/labs` — lab list (`deadlineDate`, `urgencyState`) scoped to the **current quarter** for lecturers and enrolled students; students also require `student_visible` + `release_date` (`LabDeadlineHelper.isOpenForStudentSubmission`); empty when no current quarter is set. Upload loads the lab with `findByIdWithTerm` so submit access does not lazy-load `lab.term`.

...
- `GET /api/labs` — student-facing lab list (`deadlineDate`, `urgencyState`); with a student JWT, only current-term labs that are visible and released (`student_visible` + `release_date` via `LabDeadlineHelper.isOpenForStudentSubmission`); lecturers still see all labs with `studentVisible` and `releaseDate`. Upload loads the lab with `findByIdWithTerm` so submit access does not lazy-load `lab.term`.

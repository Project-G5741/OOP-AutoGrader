...
- Lecturer-only: roster `plagiarismFlagged`, danger mark after flagged lab names, `GET /api/lecturer/plagiarism/flags` (includes `overlapByStudentAndLab` max `hashSimilarity` per student-lab), `GET /api/lecturer/labs/{labId}/plagiarism`, lab statistics `plagiarismRate` (flagged unique students ÷ students submitted)
